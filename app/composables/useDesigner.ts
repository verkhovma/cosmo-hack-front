...
export function useDesignerState() {
  ...
  const clients = computed(() => draft.value?.ground_sites.filter(g => g.role === 'client') ?? [])
...
function recordDraft(history: HistoryApi, prev: null | Scenario, next: Scenario) {
  ...
  if (prev) history.push(prev, next)
  else history.reset(next, OPEN_LABEL)
}

function createHistoryNav(apply: (s: null | Scenario) => void, history: HistoryApi) {
  function undo() {
...
export function useDesignerRunner(state: DesignerState) {
  ...
  async function runNow() {
    ...
    try {
      ...
      if (!state.selectedClient.value) {
        ...
        if (first) state.selectedClient.value = first.id
      }
    } catch (e) {
      if (my !== runToken) return
      state.computeError.value = errorMessage(e, 'Ошибка расчёта')
      toast.error('Расчёт не удался', {
        description: state.computeError.value,
      })
    } finally {
      if (my === runToken) state.busy.value = false
    }
  }
  ...
  async function refreshSnapshot() {
    ...
    try {
      ...
      if (my === snapToken) state.snapshot.value = s.snapshot
    } catch (e) {
      if (my === snapToken)
        toast.error('Не удалось получить снимок', {
          description: errorMessage(e, 'Ошибка сети'),
        })
    }
  }
...
export function useDesigner() {
  ...
  function applyDraft(s: null | Scenario) {
    if (!s) return
    state.draft.value = s
    persistDraft(s)
    scheduleRun()
  ...
  function addFailureFor(satId: string) {
    ...
    updateDraft({
      ...
      failures: [
        ...s.failures,
        { end_s: end, satellite_id: satId, start_s: start },
      ],
    })
    toast.success(`Отказ ${satId}`, {
      description: `+ [${start}; ${end}) — расчёт перезапустится`,
    })
  }
